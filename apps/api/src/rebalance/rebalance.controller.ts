...
export class RebalanceController {
  ...
  @ApiOperation({
    ...
    description:
      'Recalcula os macros do dia ao escolher uma opção diferente da default de uma refeição e devolve a prévia das refeições seguintes reescaladas (ou sem-acao quando cabe na faixa, ou recusa-orientada quando estoura o piso). Não persiste nada. recusa-orientada é HTTP 200 ("nunca barra"). Números respeitam o gate de exposição.\n\nCorpo: `{ triggerMealId, chosenOptionId, dayTypeId?, items? }`. O `dayTypeId` é o override de tipo-de-dia da sessão (opcional, mesma semântica do POST /registro): presente, o dia considerado é o desse tipo — roster, alavancas e faixa-alvo saem dele, e o `triggerMealId` deve ser uma refeição dele; ausente, vale a programação do weekday. 404 quando o tipo-de-dia não pertence ao plano ativo.\n\n`items` (020, opcional) é o overlay da edição em lote: `[{ itemId, foodId, quantityGrams }]` na MESMA forma do `consumo.items` do POST /registro — a refeição-gatilho é avaliada com essa composição editada (item sem entrada segue planejado; entradas repetidas do mesmo item somam, como na combinação). 400 para forma inválida; 404 para item fora da opção escolhida ou alimento inexistente; 422 para item travado/sem grupo. Ausente, o comportamento é byte-a-byte o de sempre.',
  })
