...
> **Deepening `012-leitura-do-registro`** (2026-07-25) — não é feature de produto: colapsa 5 implementations da leitura de `meal_event` em 2 modules empilhados. Sem migration, sem endpoint novo, **nenhuma resposta HTTP alterada**.
> **Deepening `013-construtor-de-cenario`** (2026-07-26) — infraestrutura de teste: `buildScenario` em `packages/db/src/testing/`. As suítes declaram o cenário em vez de montá-lo; −361 linhas nas 2 migradas.
...
| App | Stack | Estado |
|---|---|---|
| `apps/api` | NestJS 11 | 5 endpoints do paciente (casca fina sobre `@bamboo/core`): `GET /patients/:id/today[?dayTypeId]`, `GET /meal-items/:id/substitutions`, `POST /meal-items/:id/combine`, `POST /patients/:id/rebalance/option-choice`, `POST /patients/:id/registro` · + a via `/nutri/*` atrás do `NutriKeyGuard`: adesão (006), ciclo (007), relatório (011) e **`GET /nutri/patients`** (015 — a roster). **158 e2e** (16 specs) em `test/`. |
...
| `apps/web` | Next.js 16 (App Router) | **015:** a visão da nutri — `/` (roster) e `/patients/[id]` (o ciclo do paciente: adesão, evolução semanal, padrão de registro, comparativo). Server Components, **zero JS no cliente**, credencial só no servidor. `lib/format.ts` (derivação de apresentação, pura) + `lib/nutri.ts` (os 2 GETs); **Vitest** no app (29 testes). Sobe na **3001**. |
...
| 3 — Inteligência da nutri | registro (`003`), ciclo (`007`), adesão (`006`), auto-classificação (`008`) e **relatório de ciclo (`011`) — todos implementados e testados**; UI da nutri (web, EP-6) — **leitura entregue** (`015`); abrir/fechar ciclo e trocar plano ativo pela tela, não | **EP-5 concluído · EP-6 em curso** |
...
> **`012-leitura-do-registro` — deepening da leitura do registro (implementada e testada, 2026-07-25):** não é feature de produto — **critério de sucesso é a ausência de mudança**. Havia **5 implementations** de "qual é o registro vigente nesta janela" (`registro-consumo.ts`, `adesao/adesao-consumo.ts`, `ciclo.service.registrosDaJanela`, `relatorio.loader.ts` §1-2 e uma query inline no `plan.service.ts`), cada uma com sua ordenação, seu desempate e seu escopo de plano implícito. Viraram **2 modules empilhados** + 1 função de núcleo: `packages/core/src/registro.ts` ganhou `eventoVigente` (a redução last-write-wins devolve a **linha** vencedora, com o tipo de retorno *narrowed* pra ninguém precisar de cast — o cast apagaria o descarte do tombstone); `apps/api/src/registro-vigente.loader.ts` é o **único** leitor de `meal_event` no caminho de leitura (1 query, `ORDER BY (logged_date, created_at, id)`, `escopo` de plano **obrigatório** e sem default); `apps/api/src/consumo-real.loader.ts` **recebe** os vigentes e só resolve nutrientes (não toca `meal_event`; `meal_event_item` sim — é o snapshot do troquei). **Ganho de comportamento (único):** o `, id` no `ORDER BY` — `created_at` é `DEFAULT now()` = `transaction_timestamp()`, tomado **antes** do advisory lock do INSERT, então empate (e até inversão) é possível, e os 4 leitores antigos resolviam pelo primeiro que o heap devolvesse. **Ganho medido** (logger SQL do Drizzle ligado num servidor real, contagem por request): `GET /today?dayTypeId=` de **2 → 1** leitura de `meal_event`; `GET .../cycles/:id/report` com comparativo de **6 → 4**. **Testes que faltavam, escritos ANTES da extração** (a suíte era cega aos dois eixos, porque todo fixture usava um plano e um tipo-de-dia): `apps/api/test/escopo-plano.e2e-spec.ts`, self-contained, com fixture de **2 planos e 2 tipos-de-dia** — T-A (escopo de plano, caracterização; as asserções discriminantes são a adesão e o caminho por `position` do `/today?dayTypeId=`, porque o `/today` simples e o rebalance são cegos ao eixo: o filtro por `mealId` já exclui o plano aposentado) + T-D (janela do dia) + T-C (desempate, escrito e **visto falhar** 3× antes do leitor novo). **Fora de escopo por decisão, não por esquecimento:** Q3-B e a fonte do fallback de plano (candidato 05 — unificar muda número que a nutri já viu) e `mealId`-vs-`position` sob override ([ADR-0001](adr/0001-chave-de-pareamento-sob-override.md) + KI-002). **Sem migration, sem endpoint novo, zero mudança no mobile.** Saldo em `apps/api/src`: **−210 linhas** (465 apagadas nos 2 helpers). Artefatos novos: `CONTEXT.md` (glossário de domínio, não existia), `docs/adr/` (primeiro ADR), KI-002/003/004 em [[known-issues]].

> **`013-construtor-de-cenario` — construtor de cenário para as suítes e2e (implementada e testada, 2026-07-26):** candidato 04 da revisão de arquitetura. Infraestrutura de teste, **nenhum endpoint muda**. O gatilho foi o KI-004: a suíte e2e só passava de segunda a sexta e ninguém sabia, porque as suítes se apoiavam em detalhes do fixture de produção — inclusive o calendário. Medido no levantamento: **330** linhas de fixture em `escopo-plano`, **234** em `colisao-position`, **460** em `relatorio`; `isoDaysAgo` duplicado **byte-a-byte em 5 arquivos**; **6** assinaturas diferentes de inserir `meal_event`; a ordem reversa de FK reescrita a cada suíte nova. Entregue: **`packages/db/src/testing/scenario.ts`**, exportado pelo subpath **`@bamboo/db/testing`** (fora do barril, para não virar dependência alcançável do runtime) — 3 funções (`buildScenario`, `localDate`, `everyWeekday`) + um handle de 4 membros compram 11 tabelas, ordem de FK nos dois sentidos, atomicidade e resolução determinística de pré-requisitos. **Desenho por design-it-twice:** 3 interfaces em paralelo (declarativo / builder fluente / presets nomeados), julgadas por profundidade, localidade e risco; venceu o **declarativo** por motivo empírico — `escopo-plano` já tinha extraído à mão um `criarTipoDia(...)` e o `seed.ts` tem `insertMeal`/`insertOption`/`insertItem`, que são a árvore imperativa do **mesmo valor**. O discriminante real foi o **endereçamento**: `{dayType, position}` faz `planId`/`dayTypeId`/`patientId` do evento serem **derivados** do grafo, então o par incoerente que o KI-002 investiga é inexpressável. Os presets perderam por crescerem por chamador (shallow) e por esconderem calibração *load-bearing*. **Invariantes I-1..I-9 fazem parte da interface**, não da implementation — as duas que vieram do design-it-twice: resolver pré-requisitos **antes** do primeiro insert (spec irresolvível lança sem escrever nada ⇒ nenhum paciente órfão para os 12 `from(patient).limit(1)` sem `where` sortearem) e resolução determinística (`ORDER BY` explícito, food nunca com 0 kcal). **Migradas como prova:** `colisao-position` (506→348) e `escopo-plano` (558→355) — **−361 linhas**, zero `insert(`/`delete(`/`getDay()` nas duas. **Equivalência provada por reversão, em duas etapas:** primeiro com um shim que mantinha os `it` byte-idênticos (e asseria que os ids passados à mão eram os derivados), reversão confirmando os mesmos casos vermelhos; só então o shim saiu. Os oráculos do KI-002 e os SC-007/SC-008 da 012 derrubam exatamente os mesmos casos antes e depois. **Não migram, por decisão:** `relatorio.e2e` (a resolução determinística mudaria os denominadores nutricionais — re-derivar é indistinguível de regressão), `adesao.e2e`/`ciclo.e2e` (o `beforeAll` da adesão é **leitor**, não montagem) e as 8 suítes que **resolvem** o seed em vez de montar (outro seam: resolver, não construir). O `seed.ts` **não** foi tocado: a exigência era que ele *pudesse* ser chamador, provado pelo seam `executor` + um teste de rollback. Artefatos: `specs/013-construtor-de-cenario/` (spec/plan/tasks).

> **`015-visao-da-nutri` — a visão da nutri, parte de leitura (implementada e testada, 2026-07-26):** o **EP-6 começou**. O EP-5 estava concluído e **invisível**: 4 vias `/nutri/*` prontas (adesão 006, ciclo 007, relatório 011) e **zero consumidores** — a única forma de a nutri ver o relatório era `curl` com `x-nutri-key`. Entregue: **um** endpoint novo (`GET /nutri/patients` — a porta de entrada; sem ele é impossível chegar a um paciente sem já saber o UUID, porque toda rota da nutri é `/nutri/patients/:patientId/...`) e **duas telas** no `apps/web`, que deixou de ser boilerplate: `/` (roster, com o estado do ciclo de cada paciente) e `/patients/[id]` (adesão média dominante + evolução semana a semana + padrão de registro total e por refeição + comparativo). **Nenhuma régua nova** (FR-007): tudo vem calculado da API; `packages/core` **intocado** e nenhum endpoint existente mudou. **Server Components puros, zero JS no cliente** — é o que garante que a credencial não vá ao navegador (FR-006): env sem `NEXT_PUBLIC_`, nenhum `"use client"` no app, verificado ao vivo por `grep` da chave no HTML servido das duas telas (**0**). `cicloAtual` = o ciclo aberto; se não houver, o **fechado mais recente** (D2), resolvido pela ordem da query com `closed_on DESC NULLS FIRST` **explícito**. Minimização LGPD: a roster devolve só `id`/`name`/ciclo, travado por teste nas chaves da resposta. **A derivação que tem teste é a de apresentação** (`apps/web/lib/format.ts`, 24 testes, Vitest novo no app): o DTO do relatório **mistura escalas** — adesão `0–100`, cobertura e taxas `0–1` —, então são duas funções distintas (`pct100`/`pct01`) em vez de uma com flag; e `contarDias`/`dataCurta` não passam por `new Date(iso)`, que é meia-noite **UTC** e renderiza o dia anterior a oeste de Greenwich. **Visualização em CSS, sem lib:** a evolução semanal é um colmo de segmentos cuja **largura** é o número de dias da semana (a semana parcial é mais curta porque é mais curta; a semana sem registro aparece **hachurada**, não zerada) e o padrão de registro é uma barra empilhada; paleta categórica **validada** (banda de luminosidade, croma, ΔE de daltonismo e contraste) nos dois modos, com "sem registro" como hachura — ausência de dado não é a quarta série. **Decisão de produto embutida na cor:** o delta de `troquei` é **neutro**, nunca vermelho — trocar é adaptação, não falha. **Fora de escopo por decisão:** abrir/fechar ciclo e trocar plano ativo pela tela (são **atos** da consulta; os `POST` já existem), seletor de ciclo antigo, adesão na roster (custaria N relatórios por render), `GET /nutri/patients/:id` (a tela do paciente relê a roster — a regra de "qual ciclo mostrar" fica num só lugar) e auth real (a credencial stub dá o papel "nutri do sistema", limite v0 já declarado na 006). Sem migration; boilerplate do `create-turbo` apagado; web sobe na **3001** (a API é 3000). Artefatos: `specs/015-visao-da-nutri/` (spec/plan/tasks).

**Cobertura de teste (atual, 2026-07-26):** **164** em `packages/core` · **20** em `packages/db` (novos — o package não tinha suíte) · **158** em `apps/api` (147 + 4 da 014 + 7 de `nutri-patients`) · **24** em `apps/mobile` · **29** em `apps/web` (novos — o app não tinha suíte). _(Baseline imediatamente anterior à 012: core 157 · apps/api 132 = 119 e2e + 13 unit colocados · mobile 24 — com a ressalva de que as 132 só eram verdes **de segunda a sexta**; ver KI-004.)_
...
- **Gate de 2026-07-25 (respondido pelo dono):** `012-leitura-do-registro` **implementada e testada** — deepening, não produto. 5 implementations da leitura de `meal_event` → 2 modules empilhados + `eventoVigente` no núcleo; nenhuma resposta HTTP alterada. Baselines: **core 164 + api 139**.
- **Candidato 04 — construtor de cenário: FEITO** (feature `013`, 2026-07-26). O que sobra da classe do problema está nomeado no KI-004: as 8 suítes que **resolvem** o seed precisam de um **leitor**, que é outro seam.
- **RESOLVIDO — os dois bugs que aguardavam decisão** (feature `014`, 2026-07-26, decisão do dono = opção **(a)**; ver [ADR-0003](adr/0003-option-choice-aceita-o-override-de-tipo-de-dia.md), que supersede o ADR-0001):
  - **KI-005** ✅ — `POST option-choice` passou a aceitar um `dayTypeId` opcional, com a mesma semântica do `POST /registro`. O roster vira o do tipo exibido, o gatilho é encontrado, e o 404 morreu. A prévia de rebalanceamento voltou a ser alcançável sob override.
  - **KI-002 Sintoma A** ✅ no caminho do override — de graça no mesmo conserto: com o roster certo, o `mealId` do evento casa sozinho. **Nenhuma troca de chave foi necessária** — a pergunta original do KI ("`mealId` ou `position`?") era a errada: o defeito estava em qual **dia** o motor recebia. A opção (b) foi rejeitada por medição: verificado por reversão que ela não mata o KI-005.
  - ⚠️ **Resíduo aceito**, não escondido: (a) faz o motor seguir o tipo **exibido**, então no caminho "registrei sob B → voltei para o tipo padrão → escolho ali" o evento de B segue invisível ao motor, enquanto o `/today` com override mostra o badge por posição. Fechá-lo exigiria decidir se o `/today` do tipo padrão deveria contar evento de outro tipo — o que contradiz FR-013a da 004. Decisão separada; pinado em teste e registrado em KI-002.
  - ⚠️ **Sintoma B segue aberto** — descarte silencioso do relatório sob colisão de `position`; rota da nutri, ver ADR-0002.
- **Candidato 05** (dívida nomeada, não escondida): unificar Q3-B e a fonte do fallback de plano entre `adesao.service.ts` e `relatorio.loader.ts`. Sem bug conhecido, e unificar **muda número que a nutri já viu** — que é o próprio motivo de estar fora. Ver [ADR-0002](adr/0002-granularidade-divergente-nas-rotas-da-nutri.md).
- **Fase 3:** ~~ciclo~~ **entregue** (`007`), ~~adesão~~ **entregue** (`006`), ~~auto-classificação~~ **entregue** (`008`), ~~relatório de ciclo~~ **entregue** (`011` — EP-5 concluído). **UI da nutri (web, EP-6): a parte de LEITURA está entregue** (`015` — roster + o ciclo do paciente). Falta a parte de **ato**: abrir/fechar ciclo e trocar o plano ativo pela tela (os `POST` já existem na API), seletor de ciclo antigo e auth real (hoje a credencial stub dá o papel "nutri do sistema", então a roster não é escopada por nutri responsável).
