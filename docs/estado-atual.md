...
**Cobertura de teste (atual, 2026-07-25):** **164 testes** em `packages/core` (157 + 7 de `eventoVigente`); **139** em `apps/api` (132 + 7 de `escopo-plano`); **24** em `apps/mobile`. _(Baseline imediatamente anterior à 012, verificada ao vivo: core 157 · apps/api 132 = 119 e2e + 13 unit colocados · mobile 24 — com uma ressalva: as 132 só eram verdes **de segunda a sexta**; ver KI-004.)_
...
- **Herdado da 012 (dívida nomeada, não escondida):** o **candidato 05** da revisão de arquitetura de 2026-07-25 — unificar Q3-B e a fonte do fallback de plano entre `adesao.service.ts` e `relatorio.loader.ts`. Fica fora até haver decisão de produto, porque unificar **muda número que a nutri já viu**. Idem [ADR-0001](adr/0001-chave-de-pareamento-sob-override.md) (`mealId` vs `position` sob override): precisa de decisão de produto, com o teste de colisão escrito antes (repro em KI-002). E o **construtor de cenário** (candidato 04) — as suítes e2e ainda dependem do fixture de produção (KI-004).
