...
> **Dois bugs abertos com teste de caracterização e à espera de decisão de produto:** KI-002 (chave de pareamento sob override) e **KI-005** (a prévia de rebalanceamento está **morta** sob override — 404; descoberto 2026-07-26, não estava catalogado).
...
**Cobertura de teste (atual, 2026-07-26):** **164** em `packages/core` · **20** em `packages/db` (novos — o package não tinha suíte) · **147** em `apps/api` (132 baseline + 7 de `escopo-plano` + 8 de `colisao-position`) · **24** em `apps/mobile`. _(Baseline imediatamente anterior à 012: core 157 · apps/api 132 = 119 e2e + 13 unit colocados · mobile 24 — com a ressalva de que as 132 só eram verdes **de segunda a sexta**; ver KI-004.)_
...
- **AGUARDANDO DECISÃO DO DONO — os dois bugs com teste de caracterização já pronto:**
  - **KI-005** (novo, 2026-07-26, **prioridade alta**): a prévia de rebalanceamento devolve **404 sob override de tipo-de-dia**, para qualquer refeição, com ou sem registro. `POST option-choice` não aceita `dayTypeId` (o `POST /registro` aceita), o roster é filtrado pelo tipo do weekday, e o app renderiza os chips de opção sem gatear por override. Atinge o diferencial do produto — e é mais grave que o KI-002: lá o número sai errado, aqui a função não roda.
  - **KI-002 / [ADR-0001](adr/0001-chave-de-pareamento-sob-override.md)**: qual chave pareia o consumo sob override, `position` ou `mealId`. A precondição do ADR ("com o teste de colisão escrito **antes**") está **satisfeita** — `apps/api/test/colisao-position.e2e-spec.ts`, 8 casos, poder de detecção verificado por reversão. Falta só a decisão. A investigação de 2026-07-26 corrigiu duas coisas: o repro que estava publicado **não** demonstrava o bug (mascarava-o), e a atribuição a FR-013b da 004 era enganosa (`position` não aparece na spec da 004; o endosso em nível de spec é da 009, cuja FR-011 proíbe tocar o motor). Existem **dois consertos** com efeitos diferentes, e o **(a)** resolve os dois KIs de uma vez — detalhado no KI-002.
