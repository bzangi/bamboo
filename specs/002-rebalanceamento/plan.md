...
Entregar **o motor que adapta o resto do dia** quando o paciente desvia do plano — um motor único alimentado por três gatilhos: **(P1)** escolher uma opção desigual de uma refeição → recalcula todos os macros e espalha a diferença nas demais refeições não registradas, com **prévia antes de confirmar**; **(P2)** combinação 1→2 (preserva o nutriente-base, split 50/50 ajustável); **(P3)** troca de tipo-de-dia (regra por total-do-dia no núcleo; no app v0 só exibe o novo cardápio). Sempre com **faixa-alvo (não teto)**, **piso inviolável + recusa orientada** e **ação, não número**.
