...
- **Alimento de perfil misto** (ovo: proteína + gordura; leguminosas: carboidrato + proteína; preparações como pizza): é o caso que define a Q3 (ambiguidade/confiança). Default seguro enquanto não decidido: fica **sem grupo** e é relatado — introcável até decisão humana.
...
- **FR-001**: O sistema MUST pré-classificar automaticamente os alimentos da base nos grupos de substituição por **semelhança nutricional**, criando o vínculo alimento↔grupo que a substituição consome _(decisão "sem enlouquecer", decisoes-produto.md:89)_, via [NEEDS CLARIFICATION: Q1 — Abordagem e vigência: **(a) método** — (A) heurística determinística por perfil nutricional (macro dominante por 100 g + proximidade ao perfil do grupo): testável, explicável pra nutri, sem custo por execução. Atenção: 3 dos 4 grupos vigentes compartilham o nutriente-base carboidrato (Carboidratos, Frutas, Vegetais), então macro dominante sozinho não separa banana de arroz — a "proximidade ao perfil do grupo" precisa ser definida pra distinguir grupos de MESMO nutriente-base; (B) tabela curada manualmente uma vez: zero erro, mas não escala pra outras bases; (C) IA/LLM em lote com pipeline de revisão: cobre ambíguos melhor, mas custo + não-determinismo + revisão obrigatória antes de valer. **(b) vigência** — o vínculo automático vale **imediatamente** como opção de troca pro paciente, ou exige **revisão humana antes de valer**? Duas fontes decididas puxam em direções opostas: o Princípio V e decisoes-produto.md:85 exigem confirmação clínica antes de valer; decisoes-produto.md:89 ("o sistema pré-classifica... a nutri só mexe nas exceções") sugere valer por default. Recomendação: A + vale imediatamente (método determinístico/explicável + seed-first) — desvio consciente do espírito do Princípio V, a registrar na aprovação do dono; se a escolha for C (IA), confirmação prévia obrigatória sem exceção (Princípio V literal).]
...
- **FR-005**: Para alimentos **sem encaixe confiável** em grupo nenhum, o sistema MUST seguir [NEEDS CLARIFICATION: Q3 — Ambiguidade/confiança: (a) o alimento ambíguo fica SEM grupo (seguro: introcável até decisão humana) ou recebe o melhor palpite (mais cobertura, mais erro clínico potencial)? (b) um alimento pode ser vinculado automaticamente a MAIS de um grupo (ex.: feijão em "leguminosas" e em "fontes de proteína vegetal")? O modelo de vínculo permite, mas a validação do "troquei" no registro (Fase 3) hoje resolve UM grupo por alimento — multi-grupo exigiria corrigir essa resolução pra olhar o grupo esperado do item, custo que a recomendação de grupo único evita; (c) a porção de referência automática é derivada pra equivaler à porção típica do grupo (com guarda de plausibilidade) ou fixada por regra simples? Qualquer que seja a resposta, "encaixe confiável" e "porção plausível" MUST existir como **critério observável** (ver Assumption "Limiar de confiança/plausibilidade"). Recomendação: sem-grupo no ambíguo + um único grupo por vínculo automático + porção derivada da equivalência com guarda de plausibilidade.]
- **FR-006**: O escopo/momento da execução e a forma da taxonomia de grupos MUST seguir [NEEDS CLARIFICATION: Q2 — Escopo da execução + taxonomia + base: **(a) momento** — a classificação cobre a base inteira de uma vez (em lote, junto da ingestão/atualização da base) ou roda sob demanda (quando um alimento entra num plano)? **(b) de quem são os grupos** — esta execução classifica só nos grupos do sistema? O modelo de dados JÁ suporta grupos do sistema e grupos por-nutri coexistindo (não há mudança de modelo em jogo); o custo real de taxonomia por nutri é operacional — classificar N taxonomias. **(c) qual taxonomia** — os 4 grupos vigentes (Carboidratos, Proteínas, Frutas, Vegetais) não cobrem a TACO completa (gorduras/óleos, laticínios, bebidas, preparações ficariam honestamente sem grupo); quem define/aprova o conjunto de grupos canônicos antes da execução, e quais são? (Criar grupos é curadoria/decisão de produto, nunca palpite da execução — ver Assumption.) **(d) qual base** — a ingestão atual da TACO é uma allow-list curada de 23 alimentos (16 já vinculados); ampliar a ingestão pra TACO completa (~590 itens) faz parte desta feature, ou ela roda só sobre a base vigente (e a US1 vira quase no-op: ~7 alimentos)? Recomendação: lote sobre a base inteira + grupos do sistema (customização por nutri deferida) + taxonomia expandida aprovada pelo dono antes da execução + ampliação da ingestão TACO incluída na feature (é o que dá valor real).]
...
- **Grupo de substituição**: conjunto de alimentos equivalentes entre si por um **nutriente-base** (a troca preserva esse nutriente). O modelo já distingue grupos do sistema de grupos por-nutri; o que a Q2 decide é em quais grupos esta execução classifica (b) e qual o conjunto canônico de partida (c) — os 4 vigentes não cobrem a TACO completa.
...
- **SC-002**: Aplicada às cegas aos alimentos dos **vínculos curados vigentes** (gabarito de validação), a classificação automática coincide com o grupo curado em **pelo menos 90%** dos casos. _Número proposto, a validar pelo dono no gate: o gabarito real são 16 vínculos curados — 90% significa errar no máximo 1, estatisticamente frágil; e 3 dos 4 grupos compartilham o nutriente-base carboidrato (o caso difícil pra heurística da Q1-A). Prever ampliar o gabarito antes de tratar o % como firme. Este SC arma o gatilho de reversão da vigência (Q1b)._
...
- **SC-007**: Pelo menos **80%** dos alimentos com dados completos (mesma definição do SC-001) recebem classificação automática — a decisão humana fica restrita às exceções ("a nutri só corrige exceções"). _Número proposto, a validar pelo dono no gate; condicionado à taxonomia expandida (Q2c) e à base ampliada (Q2d) — com os 4 grupos vigentes e a TACO completa, gorduras/laticínios/bebidas/preparações ficariam honestamente sem grupo e a meta não fecha._
...
- **Vigência do vínculo automático é decisão do dono, não assumption**: "vale imediatamente vs revisão prévia" foi promovida pra Q1(b) — é o ponto onde o Princípio V/decisoes-produto.md:85 (confirmação antes de valer) e decisoes-produto.md:89 (pré-classifica por default) colidem. A recomendação "vale imediatamente (seed-first)" está registrada na Q1, com gatilho de reversão armado pelo SC-002.
...
- **Classifica dentro do conjunto de grupos vigente**: a execução encaixa alimentos nos grupos que existem; **criar grupos novos é curadoria/decisão de produto**, nunca palpite da execução — vale qualquer que seja a resposta da Q2 (qual conjunto de grupos vale na largada é a Q2c).
...
- **Base alvo = a base de alimentos vigente do produto (TACO)**: hoje, uma allow-list curada de 23 alimentos — ampliar a ingestão pra TACO completa é a Q2(d). A execução é re-executável para quando outras bases/volumes chegarem, mas integrar **outras bases** (TBCA etc.) não é desta feature.
...
- **Import de plano por IA** (Fase 4) e **ampliação para outras bases** além da TACO (TBCA etc.) — a classificação é re-executável para quando isso chegar, mas integrar outras bases não é daqui. _(Ampliar a própria ingestão TACO além da allow-list atual de 23 NÃO está excluído por este item — é a Q2d.)_
- **Mudança na matemática de substituição/equivalência/rebalanceamento** — a mecânica é a mesma; só a cobertura cresce.
- **Criação/edição da taxonomia de grupos pela nutri** — depende da Q2; se a taxonomia for canônica, edição nem existe.
