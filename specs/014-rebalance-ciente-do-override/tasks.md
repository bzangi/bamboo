...
- [ ] **T001** Registrar o baseline das 4 suítes.
- [ ] **T002** **[TDD — VER FALHAR]** Inverter os 2 casos do bloco `KI-005` em
...
- [ ] **T003** **[TDD — VER FALHAR]** Caso novo da **US2**: registro `pulei` na pos 1 do tipo B
...
- [ ] **T004** **[TDD — VER FALHAR]** Casos novos da **US3**: (a) `dayTypeId` que não é do plano
...
- [ ] **T005** Rodar a suíte e confirmar que T002/T003/T004 estão **vermelhos** e os outros
      casos verdes.
...
- [ ] **T006** `packages/types/src/rebalance.ts`: `OptionChoiceRequest` ganha
...
- [ ] **T007** `apps/api/src/rebalance/rebalance.service.ts` bloco 4: resolver o tipo-de-dia do
...
- [ ] **T008** `rebalance.controller.ts`: documentar o campo no Swagger (`@ApiBody`/modelo),
      espelhando como o `/registro` documenta o dele.
- [ ] **T009** Rodar a suíte completa. T002/T003/T004 verdes; **`rebalance.e2e-spec.ts` verde
      com `git diff` vazio** — é a prova de FR-003, e a suíte tem 15 casos calibrados no seed.
- [ ] **T010** Reescrever o comentário do caso do **resíduo A2** (gatilho em **A**, registro em
...
- [ ] **T011** `RebalancePreviewSheet` ganha a prop `dayTypeId?: string` e a envia no corpo.
...
- [ ] **T012** `HomeScreen` passa o `dayTypeId` do override (o mesmo estado que alimenta
      `overrideActive`) para a sheet.
- [ ] **T013** `pnpm --filter mobile test` (24) + `tsc` do mobile (exige `pnpm build` antes) +
      lint.
...
- [ ] **T014** Verificar os SCs com comando: **SC-003** `git diff` vazio nas 11 suítes não
...
- [ ] **T015** Regenerar o OpenAPI. Done-gate: `pnpm lint` + `pnpm format` + `pnpm check-types`.
- [ ] **T016** **Superseder o ADR-0001** com um ADR novo que registra a decisão (a), por que (b)
      foi rejeitada (não mata o KI-005 — verificado por reversão) e o resíduo A2. O ADR-0001
      passa a `superseded`.
- [ ] **T017** Fechar **KI-005** (resolvido) e atualizar **KI-002**: o Sintoma A morre no
